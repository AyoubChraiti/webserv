...
mpserv configChecking(const string &filePath) {
    ...
    for (map<string, vector<servcnf> >::const_iterator it = config.servers.begin(); it != config.servers.end(); ++it) {
        const vector<servcnf> &serverList = it->second;

        for (size_t i = 0; i < serverList.size(); ++i) {
            const servcnf &server = serverList[i];
            ...
            for (map<int, string>::const_iterator err_it = server.error_pages.begin(); err_it != server.error_pages.end(); ++err_it) {
            ...
            for (map<string, routeCnf>::const_iterator route_it = server.routes.begin(); route_it != server.routes.end(); ++route_it) {
                ...
                for (vector<string>::const_iterator method_it = route.methodes.begin(); method_it != route.methodes.end(); ++method_it) {
